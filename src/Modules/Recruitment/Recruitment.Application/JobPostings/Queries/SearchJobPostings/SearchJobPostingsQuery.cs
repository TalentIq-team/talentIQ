    ...
    : IRequestHandler<SearchJobPostingsQuery, IReadOnlyList<JobPostingDto>>
{
    ...
    {
        var anyJobs = await _db.JobPostings.AnyAsync(cancellationToken);
        if (!anyJobs)
        {
        ...
        {
            var unpublished = await _db.JobPostings.Where(j => j.Status == JobPostingStatus.Draft).ToListAsync(cancellationToken);
            if (unpublished.Any())
            {
                foreach (var d in unpublished)
                {
                    d.Publish();
                }
                await _db.SaveChangesAsync(cancellationToken);
