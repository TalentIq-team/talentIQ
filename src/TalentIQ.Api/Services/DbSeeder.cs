...
public static class DbSeeder
{
    public static async Task SeedDatabaseAsync(IServiceProvider services, ILogger logger)
    {
        ...
        var usersToSeed = new (string Email, UserRole Role, Guid? DeptId)[]
        {
            ...
            ("michael.chen@talentiq.dev", UserRole.Candidate, null),
        ...
        if (!await analyticsDb.TalentPoolEntries.AnyAsync())
        {
            var poolEntries = new List<TalentPoolEntry>
            {
                ...
                new() { Id = Guid.NewGuid(), CandidateProfileId = profileJohn.Id, AddedByRecruiterId = recruiterUser.Id, ConsentStatus = ConsentStatus.Accepted, SkillTags = "[\"Python\", \"Gemini API\", \"C#\", \"Machine Learning\"]", ProfileSnapshotJson = "AI & Backend Specialist with 6 years experience. Expert in Gemini API integration.", CreatedAt = DateTime.UtcNow.AddDays(-18), ConsentRespondedAt = DateTime.UtcNow.AddDays(-17), ConsentExpiryDate = DateTime.UtcNow.AddMonths(12), IsActive = true },
                new() { Id = Guid.NewGuid(), CandidateProfileId = profileJane.Id, AddedByRecruiterId = lisaRecruiter.Id, ConsentStatus = ConsentStatus.Accepted, SkillTags = "[\"React\", \"TypeScript\", \"Figma\", \"TailwindCSS\"]", ProfileSnapshotJson = "Frontend Architect with 4 years experience. Expert in design systems.", CreatedAt = DateTime.UtcNow.AddDays(-15), ConsentRespondedAt = DateTime.UtcNow.AddDays(-14), ConsentExpiryDate = DateTime.UtcNow.AddMonths(12), IsActive = true },
                new() { Id = Guid.NewGuid(), CandidateProfileId = profileMichael.Id, AddedByRecruiterId = recruiterUser.Id, ConsentStatus = ConsentStatus.Accepted, SkillTags = "[\"AWS\", \"Kubernetes\", \"Docker\", \"Go\"]", ProfileSnapshotJson = "Principal Cloud Architect with 8 years experience. Multi-cloud expert.", CreatedAt = DateTime.UtcNow.AddDays(-14), ConsentRespondedAt = DateTime.UtcNow.AddDays(-13), ConsentExpiryDate = DateTime.UtcNow.AddMonths(12), IsActive = true },
                new() { Id = Guid.NewGuid(), CandidateProfileId = profileSarah.Id, AddedByRecruiterId = recruiterUser.Id, ConsentStatus = ConsentStatus.Accepted, SkillTags = "[\"Python\", \"Machine Learning\", \"TensorFlow\"]", ProfileSnapshotJson = "Senior Data Scientist with 5 years experience. Published ML researcher.", CreatedAt = DateTime.UtcNow.AddDays(-12), ConsentRespondedAt = DateTime.UtcNow.AddDays(-11), ConsentExpiryDate = DateTime.UtcNow.AddMonths(12), IsActive = true },
                new() { Id = Guid.NewGuid(), CandidateProfileId = profileDavid.Id, AddedByRecruiterId = lisaRecruiter.Id, ConsentStatus = ConsentStatus.Pending, SkillTags = "[\"Docker\", \"Kubernetes\", \"AWS\", \"Go\"]", ProfileSnapshotJson = "Senior DevOps Engineer with 7 years experience. CI/CD and SRE specialist.", CreatedAt = DateTime.UtcNow.AddDays(-5), IsActive = true },
                new() { Id = Guid.NewGuid(), CandidateProfileId = profileEmma.Id, AddedByRecruiterId = lisaRecruiter.Id, ConsentStatus = ConsentStatus.Pending, SkillTags = "[\"React Native\", \"TypeScript\", \"Mobile\"]", ProfileSnapshotJson = "Mobile Developer with 4 years experience. 500K+ app downloads.", CreatedAt = DateTime.UtcNow.AddDays(-3), IsActive = true },
                new() { Id = Guid.NewGuid(), CandidateProfileId = profileJames.Id, AddedByRecruiterId = lisaRecruiter.Id, ConsentStatus = ConsentStatus.Declined, SkillTags = "[\"Selenium\", \"Java\", \"Python\"]", ProfileSnapshotJson = "Senior QA Engineer with 6 years experience. ISTQB certified.", CreatedAt = DateTime.UtcNow.AddDays(-10), ConsentRespondedAt = DateTime.UtcNow.AddDays(-9), IsActive = false },
            };
